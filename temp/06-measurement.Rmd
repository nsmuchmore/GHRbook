...
```{r measure, fig.cap="Source: http://bit.ly/1X5yjU2", echo=F}
knitr::include_graphics(rep("images/measure.jpg"))
```
...
```{r logmod, echo=FALSE}
  log <- data.frame(logic=c("Input", "Activities", "Output", 
                            "Outcomes", "Impacts"),
                    concept=c("Financial resources",
                              "Treatment fidelity",
                              "Compliance",
                              "Fewer complications",
                              "Reduced mortality"),
                    indicator=c("Expenditures",
                                "Checklist score",
                                "# TBAs trained",
                                "# hemorrhage",
                                "# maternal death"))
  #names(log) <- NULL
  knitr::kable(log, col.names=c("Logic Model", "Concept", "Indicator"))
```
...
```{r smart, echo=FALSE}
  smart <- data.frame(v1=c("**S**ecific", "**M**easurable", "**A**ttainable",
                           "**R**elevant", "**T**ime-bound"),
                      v2=c("clearly defined", "able to be quantified",
                           "target must be realistic", 
                           "related to the construct",
                           "observed at specific times"))
  names(smart) <- NULL
  knitr::kable(smart, col.names=NA)
```

I think indicators should be DREAMY™:

```{r dreamy, echo=FALSE}
  dreamy <- data.frame(v1=c("**D**efined", "**R**elevant", "**E**xpedient", 
                            "**A**ccurate", "**M**easurable",
                            "customar**Y**[^free]"),
                      v2=c("clearly specified", "related to the construct",
                           "feasible to obtain", "valid measure of construct",
                           "able to be quantified", "recognized standard"))
  names(dreamy) <- NULL
  knitr::kable(dreamy, col.names=NA)
```
...
> Maternal deaths were defined as death of the mother during pregnancy, delivery, and up to six weeks post-partum, excluding deaths known to have been due to injury or accident.
...
> In cases of maternal death, the cause was ascertained by Lady Health Workers on the basis of oral reports from relatives, neighbors, or traditional birth attendants.
...
```{r grl, fig.cap="Source: http://bit.ly/1NgGeLh", echo=F}
knitr::include_graphics(rep("images/grl.png"))
```

In the entry for maternal mortality ratio, we find the following definition:

> The annual number of female deaths from any cause related to or aggravated by pregnancy or its management (excluding accidental or incidental
...
```{r cm, fig.cap="Confusion matrix", echo=F}
knitr::include_graphics(rep("images/cm.png"))
```
...
```{r sen, fig.cap="Sensitivity", echo=F}
knitr::include_graphics(rep("images/sen.png"))
```
...
```{r spe, fig.cap="Specificity", echo=F}
knitr::include_graphics(rep("images/spe.png"))
```
...
```{r roc, fig.cap="ROC. Source: http://1.usa.gov/1EHevjO", echo=F}
knitr::include_graphics(rep("images/roc2.png"))
```
...
```{r quiz04, echo=FALSE, fig.cap="Example quiz.", eval=FALSE}
